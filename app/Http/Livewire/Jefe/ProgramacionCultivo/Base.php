    ...
    public function render()
    {
        ...
        if ($this->sede_id > 0 && $this->fecha != "" && $this->turno != "") {
            ...
            if ($this->cultivo_fundo_id == '0,0') {
                $subquery = `(SELECT COUNT(*) FROM programacion_de_tractors pt
                INNER JOIN tractors tt ON tt.id = pt.tractor_id
                WHERE pt.esta_anulado = 0 AND tt.sede_id = $this->sede_id
                AND tt.cultivo_id = tractors.cultivo_id
                AND COALESCE(tt.fundo_id,0) = COALESCE(tractors.fundo_id,0)
                AND pt.fecha = '$this->fecha'
                ...
                $tractores_por_cultivo = DB::table('tractors')
                    ->select(
                        ...
                        'cultivos.cultivo',
                        'fundos.fundo',
                        DB::raw("COUNT(*) as tractors"),
                        DB::raw("(SELECT COUNT(*) FROM programacion_de_tractors pt INNER JOIN tractors tt ON tt.id = pt.tractor_id WHERE pt.esta_anulado = 0 AND tt.sede_id = " . $this->sede_id . " AND tt.cultivo_id = tractors.cultivo_id AND COALESCE(tt.fundo_id,0) = COALESCE(tractors.fundo_id,0) AND pt.fecha = '" . $this->fecha . "' AND pt.turno = '" . $this->turno . "'" . $filtrar_por_supervisor . ") as programado")
                    )
